...
Use socket to communicate through TCP and UDP between applications. Sockets provide the communication mechanism between two computers. A client program creates a socket on its end of the communication and attempts to connect that socket to a server.

There are two common network protocols: TCP and UDP.

## Common protocal used in socket
